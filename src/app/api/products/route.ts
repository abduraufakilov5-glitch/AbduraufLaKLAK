...
const variantSchema = z.object({
  ...
  minimum_quantity: z.number().int().nonnegative().default(0),
});
...
const createSchema = z.object({
  ...
  marketplace_title: z.string().max(500).optional(),
  marketplace_description: z.string().max(12000).optional(),
  image_prompt: z.string().max(12000).optional(),
...
export async function POST(request: Request) {
  ...
  if (!profile || !["ADMIN", "CONTENT_MANAGER", "WAREHOUSE_MANAGER"].includes(String(profile.role))) {
