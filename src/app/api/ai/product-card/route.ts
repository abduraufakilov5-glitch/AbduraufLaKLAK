...
const inputSchema = z.object({
  ...
  sourceImagePath: z.string().max(300).optional(),
...
export async function POST(request: NextRequest) {
  ...
  if (parsed.data.sourceImagePath && !parsed.data.sourceImagePath.startsWith(`${user.id}/`)) {
  ...
  try {
    ...
    const { error } = await supabase.from("ai_generations").insert({
      ...
      prompt_version: "product-content-v1",
      status: "completed",
      output: result.data,
      source_image_path: parsed.data.sourceImagePath ?? null,
      created_by: user.id,
    });
